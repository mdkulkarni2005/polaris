...
const validateInternalKey = (key: string) => {
  ...
  if (!internalKey) {
    throw new Error("POLARIS_CONVEX_INTERNAL_VALUE is not configured");
  }
...
export const createFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      throw new Error("File alredy exists");
    }
...
export const updateImportsStatus = mutation({
  args: {
    internalKey: v.string(),
    projectId: v.id("projects"),
    status: v.optional(
...
export const getProjectFileWithUrls = query({
  args: {
    internalKey: v.string(),
    projectId: v.id("projects"),
  },
