...
export const UseWebContainer = ({
  ...
  const conternRef = useRef<WebContainer | null>(null);
  ...
  useEffect(() => {
    if (!enabled || !files || file.length === 0 || hasStartedRef.current) {
      return;
    }
  ...
  useEffect(() => {
    const container = conternRef.current;
