...
export const processMessage = inngest.createFunction(
      ...
      if (internalKey) {
        await step.run("update-message-on-failure", async () => {
          await convex.mutation(api.system.updateMessageContent, {
            ...
            messageId,
            content:
              "My apologies, I encountered an error while processing you request. Let me know if you need anything else!",
          });
        });
      }
    },
  ...
  async ({ event, step }) => {
    ...
    // Fetch recent messages for converstaion context
    ...
    // Build system prompt with convesation history (exclude the current processing message)
    let systemPrompt = CODING_AGENT_SYSTEM_PROMPT;

    // Filter our the current processing messages and empty messages
    ...
    if (contextMessages.length > 0) {
      ...
      systemPrompt += `\n\n## Previous Converstaion (for context only - do NOT repeat these responses):\n${historyText}\n\n## Current REquest:\nResponse ONLY to user's new message below. Do not repeat or reference your previous responses.`;
    }
