...
export const createCreateFilesTool = ({
  ...
  return createTool({
    name: "createFiles",
    description:
      "Create multiiple files at once in teh same folder. Use this to batch create files that share the same parent folder. More efficient than creating files one by one.",
    parameters: z.object({
      parentId: z
        .string()
        .describe(
        ...
        return await toolStep?.run("create-files", async () => {
          ...
          if (parentId && parentId !== "") {
            try {
              ...
              return `Error: Invalid parentId "${parentId}". Use listfiles to get valid folder IDs, or use empty for root level.`;
            }
          }
