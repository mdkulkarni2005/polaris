...
const generatePayload = (view: EditorView, fileName: string) => {
  ...
  const totalLine = view.state.doc.lines
  const lineToFetch = Math.min(5, totalLine - currentLine.number)
  ...
  return {
    ...
    lineNumber: currentLine.number, // Fix this later
  }
}

const createDebounePlugin = (fileName: string) => {
...
export const suggestion = (fileName: string) => [
  ...
  createDebounePlugin(fileName), // Triggers suggestion on typing
