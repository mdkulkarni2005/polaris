...
export const importGithubRepo = inngest.createFunction(
  {
    ...
    onFailure: async ({ event, step }) => {
      ...
      await step.run("set-failed-status", async () => {
        await convex.mutation(api.system.updateImportsStatus, {
          internalKey,
          projectId,
          status: "failed",
        });
  ...
  async ({ event, step }) => {
    ...
    //Clean any existing files in the project
    ...
    // Sort folder by depth so parents are created before children
    // Input: [{path: "src/componets"}, { path: "src" }, { path: "src/componets/ui" }]
    // Input: [{path: "src"}, { path: "src/componets" }, { path: "src/componets/ui" }]
    ...
    // (Inngest serialized step results, so we use a plain object insted of Map)
    ...
    // Get al files (blobs) from the tree
    ...
    await step.run("create-files", async () => {
      for (const file of allFiles) {
        ...
        try {
          ...
          const name = pathParts.pop()!;
          const parentPath = pathParts.join("/");
          const parentId = pathParts ? folderIdMap[parentPath] : undefined;
